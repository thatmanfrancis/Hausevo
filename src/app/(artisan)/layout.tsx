...
import { ARTISAN_NAV_ITEMS } from "@/lib/nav-constants";
...
export default async function ArtisanLayout({ children }: { children: React.ReactNode }) {
  ...
  // Temporarily bypass onboarding check for artisan if needed, or keep it if onboarding is shared
  if (!user?.onboardingCompleted) redirect("/onboarding");
  if (!user.roles.includes("ARTISAN")) {
    if (user.roles.includes("ADMIN")) redirect("/admin/dashboard");
    if (user.roles.includes("LANDLORD")) redirect("/landlord/dashboard");
    redirect("/dashboard");
  }
    ...
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-6 py-8 flex gap-8">
        ...
        <aside className="hidden md:flex flex-col w-52 shrink-0">
          <SidebarNav items={ARTISAN_NAV_ITEMS} />
        </aside>
      ...
      <MobileNav items={ARTISAN_NAV_ITEMS} />
    </div>
  );
}
