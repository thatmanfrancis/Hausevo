...
export default async function LandlordDashboardPage() {
  ...
  const [properties, applications, tenancies, notifications] =
    await Promise.all([
  ...
  const landlordInfo = await prisma.user.findUnique({
    where: { id: session.user.id },
    select: { shackScore: { select: { score: true } } },
  });
  ...
  const monthlyYield = recentTransactions.reduce(
    (acc: any, t: any) => acc + t.amount,
    0,
  );
  ...
  return (
    <LandlordDashboard
      ...
      stats={{
        ...
        monthlyYield,
        shackScore: landlordInfo?.shackScore?.score ?? 0,
      }}
